...
def findAruco(frame, arucoId, matrix_coefficients, distortion_coefficients):
    ...
    #           print(rvec,tvec) ovo sam zakomentiro da nas ne buni
                if rvec is not None and tvec is not None:
                    ...
                    target = np.array([tvec[0][0], yawD],  dtype=object)
              #     target_formatted = "[{:.6f}, {:.6f}]".format(*list(target[:2]))
                #    return np.array([tvec[0][0], yawD],  dtype=object)
                    return [tvec[0][0], yawD]
...
def trackAruco(myDrone, pid, pError, ierror, target, target_y):
    ...
    ## PID
    speed = [0, 0, 0, 0]
    error = [target[0], target[1], (target[2]*100 - myDrone.get_height() + 0.2), target_y]
